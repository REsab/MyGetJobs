 ...
 */
public class Boss11 {
	private static final Logger log = LoggerFactory.getLogger(Boss11.class);
		...
		while (!shouldBreak) {
			...
			items.forEach(info -> {
				try {
					...
					boolean match = message.contains("不") || message.contains("感谢") || message.contains(
							"但") || message.contains("遗憾") || message.contains("需要本") || message.contains("对不");
			...
			try {
				WAIT.until(ExpectedConditions.presenceOfElementLocated(
						By.xpath("//div[contains(text(), '滚动加载更多')]")));
